...
DROP FUNCTION IF EXISTS join_game_atomic(uuid, text);

/**
 * Atomically joins a student device to a game as a new team.
 *
 * Acquires a row-level lock on the game record so that concurrent invocations
 * are serialized. Within the same transaction it:
 *   1. Validates the game exists and is joinable
 *   2. Counts existing teams and enforces the num_teams capacity
 *   3. Assigns the next sequential team_number (MAX + 1)
 *   4. Resolves the team name from games.team_names[team_number - 1], falling
 *      back to "Team N" when the array is absent or too short
 *   5. Inserts the team record with connection_status = 'pending'
 *
 * Returns JSONB:
 *   Success: { "success": true, "team_id": "<uuid>", "team_number": <int> }
 *   Failure: { "success": false, "error_code": "game_not_found" | "game_completed" | "game_full" }
 *
 * @param p_game_id   UUID of the game to join
 * @param p_device_id Browser-generated device identifier for deduplication
 */
...
COMMENT ON FUNCTION join_game_atomic(uuid, text) IS
  'Atomically joins a student to a game. Locks the game row to serialize concurrent joins, '
  'enforces capacity, assigns the next sequential team_number, resolves the team name from '
  'games.team_names, and inserts the team in a single transaction.';

-- Students are anonymous (not authenticated), so grant to both roles
GRANT EXECUTE ON FUNCTION join_game_atomic(uuid, text) TO anon;
GRANT EXECUTE ON FUNCTION join_game_atomic(uuid, text) TO authenticated;
