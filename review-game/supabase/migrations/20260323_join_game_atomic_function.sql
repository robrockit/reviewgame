 ...
 *   5. Resolves the team name from games.team_names[team_number - 1], falling
 *      back to "Team N" when the array is absent or too short
 *   6. Inserts the team record with connection_status = 'pending'
 *
 * Known limitation: team_number assignment uses MAX + 1, not COUNT + 1.
 ...
 * mode — using COUNT + 1 would risk a unique constraint violation when
 * the incremented value collides with an existing row.
 *
 * Returns JSONB:
 ...
 * @param p_device_id Browser-generated device identifier for deduplication (max 255 chars)
 */
