...
export async function POST(
  ...
  try {
    ...
    // are anonymous so anon role lacks EXECUTE on submit_final_jeopardy_answer. Keeping the
    // service role scoped to the RPC means we never grant anon execute (which would let
    // anyone with the public anon key overwrite any team's answer by guessing a team UUID).
